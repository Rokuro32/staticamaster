...
function SeriesMode() {
  ...
  return (
    <div className="space-y-4">
      <canvas ref={canvasRef} width={700} height={420} className="w-full max-w-[700px] mx-auto bg-white rounded-lg border border-stone-200" />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">ε (V)</label>
          <input type="range" min={1} max={24} step={0.5} value={emf} onChange={e => setEmf(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{emf.toFixed(1)} V</span>
        </div>
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">Nombre de R</label>
      ...
      <div className="bg-ocre-50 border border-ocre-200 rounded-lg p-4">
        <h4 className="font-semibold text-ocre-900 mb-2">Résultats — Résistances en série</h4>
...
function ParallelMode() {
  ...
  return (
    <div className="space-y-4">
      <canvas ref={canvasRef} width={700} height={380} className="w-full max-w-[700px] mx-auto bg-white rounded-lg border border-stone-200" />
      ...
      <div className="bg-ocre-50 border border-ocre-200 rounded-lg p-4">
        <h4 className="font-semibold text-ocre-900 mb-2">Résultats — Résistances en parallèle</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
          <div>
...
function MixedMode() {
  ...
  return (
    <div className="space-y-4">
      <canvas ref={canvasRef} width={700} height={400} className="w-full max-w-[700px] mx-auto bg-white rounded-lg border border-stone-200" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">R₁ série (Ω)</label>
          <input type="range" min={10} max={1000} step={10} value={rSeries} onChange={e => setRSeries(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{rSeries} Ω</span>
        </div>
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">R₂ parallèle (Ω)</label>
          <input type="range" min={10} max={1000} step={10} value={rPar1} onChange={e => setRPar1(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{rPar1} Ω</span>
        </div>
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">R₃ parallèle (Ω)</label>
          <input type="range" min={10} max={1000} step={10} value={rPar2} onChange={e => setRPar2(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{rPar2} Ω</span>
        </div>
      </div>

      <div className="bg-ocre-50 border border-ocre-200 rounded-lg p-4">
        <h4 className="font-semibold text-ocre-900 mb-2">Résultats — Circuit mixte (R₁ en série + R₂ ∥ R₃)</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">ε - f.é.m. (V)</label>
          <input type="range" min={1} max={24} step={0.5} value={emf} onChange={e => setEmf(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{emf.toFixed(1)} V</span>
        </div>
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">r interne (Ω)</label>
          <input type="range" min={0.1} max={50} step={0.1} value={rInt} onChange={e => setRInt(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{rInt.toFixed(1)} Ω</span>
        </div>
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">R charge (Ω)</label>
          <input type="range" min={1} max={1000} step={1} value={rLoad} onChange={e => setRLoad(+e.target.value)} className="w-full" />
          <span className="text-sm text-stone-500">{rLoad} Ω</span>
        </div>
      </div>
      ...
      <div className="bg-prune-50 border border-prune-200 rounded-lg p-4">
        <h4 className="font-semibold text-prune-900 mb-2">Résultats — Pile réelle</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
          ...
          <div className="md:col-span-2 mt-2 p-2 bg-white rounded border border-prune-100">
...
function VIGraph({ emf, rInt, currentPoint, vPoint }: { emf: number; rInt: number; currentPoint: number; vPoint: number }) {
  ...
  return (
    <div>
      <canvas ref={canvasRef} width={500} height={280} className="w-full max-w-[500px] mx-auto bg-white rounded-lg border border-stone-200" />
    </div>
  );
}
        ...
        {(Object.keys(MODE_LABELS) as SimulationMode[]).map((m) => (
          <button
            ...
            className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors ${
              mode === m
                ? 'bg-ocre-50 text-ocre-700 border-b-2 border-ocre-500'
