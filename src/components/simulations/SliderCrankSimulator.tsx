...
export function SliderCrankSimulator() {
  ...
  const drawFrame = useCallback((ctx: CanvasRenderingContext2D, curXA: number, curYB: number) => {
    ...
    // ---- Repère global O ----
    if (showRefO) {
      drawFrame2(ctx, ox, oy, 0, '#aba6a1', 'O', 50);
    }

    // ---- Cercle trajectoire de B vu depuis A (en mode refA) ----
    if (isRefA) {
      ctx.strokeStyle = 'rgba(202, 104, 74, 0.25)';
      ctx.lineWidth = 1;
      ctx.setLineDash([6, 4]);
      ctx.beginPath();
    ...
    if (showRefB) {
      drawFrame2(ctx, Bx, By, 0, '#ca684a', 'R_B', 40);
    }
    ...
    // Badge mode de vue
    ctx.fillStyle = isRefA ? '#c29851' : '#aba6a1';
    ctx.font = 'bold 12px sans-serif';
    ctx.textAlign = 'left';
      ...
      <div className="flex flex-col items-center space-y-4">
        ...
        <div className="w-full max-w-[700px] space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm font-mono text-stone-900 w-20 text-right">{rodLength} px</span>
          </div>
          ...
          {!animate && (
            <>
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-mono text-stone-900 w-20 text-right">{xA.toFixed(0)} px</span>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-mono text-stone-900 w-20 text-right">{yB.toFixed(0)} px</span>
              </div>
            </>
          )}
          ...
          {animate && (
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-mono text-stone-900 w-20 text-right">×{animSpeed.toFixed(1)}</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <span className="text-sm text-stone-700 font-medium">Référentiels :</span>
            {[
              { label: 'R₀ (global)', checked: showRefO, set: setShowRefO, color: 'accent-stone-500' },
              { label: 'R_A (coulisseau A)', checked: showRefA, set: setShowRefA, color: 'accent-gold-500' },
              { label: 'R_B (coulisseau B)', checked: showRefB, set: setShowRefB, color: 'accent-brun-500' },
              { label: 'Trace P', checked: showTraceP, set: setShowTraceP, color: 'accent-ocre-500' },
            ].map(r => (
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <ul className="list-disc list-inside text-stone-700 space-y-1">
            <li><strong className="text-gold-600">A</strong> : translation horizontale (axe x) sur la glissière horizontale.</li>
            <li><strong className="text-brun-600">B</strong> : translation verticale (axe y) sur la glissière verticale.</li>
          </ul>
          <p className="text-stone-700">
        ...
        >
          ...
          <ul className="list-disc list-inside text-stone-700 space-y-1">
            ...
            <li>
              <strong>R_A</strong> (or) : référentiel lié au coulisseau A. Vue depuis A,
              le point B décrit un mouvement circulaire de rayon L autour de A.
            </li>
            <li>
              <strong>R_B</strong> (brun) : référentiel lié au coulisseau B. Vue depuis B,
